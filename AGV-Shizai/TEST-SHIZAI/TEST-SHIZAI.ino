...
const int pin_tin_hieu_15 = 48; // ĐÃ SỬA: Chân xuất tín hiệu cho Case 15 là 48
...
const unsigned long thoi_gian_bao_Case_15 = 10000; // ĐÃ SỬA: 10 giây xuất tín hiệu LOW
...
// PID ĐÃ ĐƯỢC TỐI ƯU CHỐNG KHỰNG
...
// BIẾN QUẢN LÝ CASE 15 CÓ CHỐNG NHIỄU
unsigned long t_bat_dau_15 = 0;
bool dang_bao_15 = false;
unsigned long t_bat_dau_xac_nhan_15 = 0; // Bộ đếm thời gian dẫm vạch
bool dang_xac_nhan_15 = false;           // Cờ hiệu đang dẫm vạch
...
void setup()
{
    ...
    // Khởi tạo chân 48 mặc định HIGH
    ...
    int read_sen = In_SenSor();
    if (read_sen == 14 || read_sen == 15)
    {
...
void KhoiTaoThongSoDeBa()
{
    ...
    dang_xac_nhan_15 = false; // Reset luôn bộ đếm nhiễu
}
...
bool KiemTraAnToanGap()
{
    ...
    else 
    {
        ...
        {
            KhoiTaoThongSoDeBa(); 
            trang_thai_14 = CHAY_BINH_THUONG; // Đảm bảo không bị lướt trạm sau khi né
...
void DocVaLocCamBien()
{
    ...
    // Khai báo 15 là mã đặc biệt để vô lăng không bị lạng lách
    ...
    if (!is_special_code)
    {
        ...
        // Trí nhớ vô lăng khi lướt rách line
        if (raw_sensor == 13) {
...
void XuLyMayTrangThai()
{
    ...
    // Bịt mắt 9 giây đầu khi khởi động
    if (now - thoi_gian_cap_nguon < thoi_gian_tang_toc) {
        if(raw_sensor == 14 || raw_sensor == 15) return;
    }

    // ================= XỬ LÝ CASE 15 (CHỐNG NHIỄU 0.5S + XUẤT LOW 10S CHÂN 48) =================
    if (raw_sensor == 15 && !dang_bao_15) 
    {
        if (!dang_xac_nhan_15) 
        {
            dang_xac_nhan_15 = true;
            t_bat_dau_xac_nhan_15 = now; // Bắt đầu đếm nhẩm
        }
        else if (now - t_bat_dau_xac_nhan_15 >= 500) // Đã dẫm vạch liên tục đủ 0.5s
        {
            dang_bao_15 = true;
            t_bat_dau_15 = now;
            digitalWrite(pin_tin_hieu_15, LOW); // Xuất LOW ra chân 48
            dang_xac_nhan_15 = false; // Reset cờ để dùng cho lần sau
        }
    }
    else if (raw_sensor != 15)
    {
        // Nếu nhấc ra khỏi vạch hoặc nhiễu nháy < 0.5s thì tự động hủy đếm
        dang_xac_nhan_15 = false;
    }

    // Kiểm tra để ngắt chân 48 sau 10s
    if (dang_bao_15 && (now - t_bat_dau_15 >= thoi_gian_bao_Case_15)) 
    {
        dang_bao_15 = false;
        digitalWrite(pin_tin_hieu_15, HIGH); // Tắt tín hiệu chân 48
    }
    // =========================================================================================
...
{
    // Cấm xuất PID (Dừng xe) khi đang bốc/thả hàng. (CASE 15 xe vẫn chạy qua bình thường)
...
void TinhToanVaXuatPID()
{
    ...
    // BỘ LỌC CHỐNG GIẬT 1: Làm mềm lỗi
    ...
    // BỘ LỌC CHỐNG GIẬT 2: Ép xung đạo hàm
    ...
    // BỘ LỌC CHỐNG GIẬT 3: Low-pass filter và Slew Rate Limiter cho động cơ bước
