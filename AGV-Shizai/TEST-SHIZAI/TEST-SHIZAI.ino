...
enum TrangThaiTram { CHAY_BINH_THUONG=0, BAT_DAU_GIAM_TOC=1, CHO_DUNG=2, DANG_DUNG_LAY_HANG=3, ROI_KHOI_TRAM=4, QUAY_DAU=5, TAM_DUNG_DE_THA=6 };
...
unsigned long thoi_gian_mat_line = 0, t_bat_dau_tam_dung = 0, t_bat_dau_tha = 0, thoi_gian_roi_tram = 0;
bool dang_tha_hang = false, dang_mat_line = false, dang_dung_vi_vat_can = false, dang_dung_vi_mat_line = false;
...
void XuLyNamCham() {
    if (dang_tha_hang && (millis() - t_bat_dau_tha >= thoi_gian_tha_hang)) {
        dang_tha_hang = false;
...
void XuLyMayTrangThai() {
    ...
    if ((trang_thai_14 == BAT_DAU_GIAM_TOC || trang_thai_14 == CHO_DUNG) && (now - thoi_gian_bat_dau_giam > time_tang_toc_case14_lan_1)) {
        trang_thai_14 = TAM_DUNG_DE_THA; t_bat_dau_tam_dung = now;
        dang_tha_hang = true; t_bat_dau_tha = now;
    ...
    if (trang_thai_14 == TAM_DUNG_DE_THA && (now - t_bat_dau_tam_dung >= 500)) {
        ...
        toc_do_khi_bat_dau_tang = smoothBaseSpeed; thoi_gian_bat_dau = now;
