...
const float Kp_789 = 200.0; const float Kd_789 = 800.0;
const float Ki = 0.01;
...
void loop()
{
    ...
    // Tách độc lập 2 hệ thống an toàn
    bool dung_vat_can = KiemTraAnToanGap(); 
    bool dung_mat_line = XuLyMatLine();     
    
    // Nếu gặp nguy hiểm (Vật cản HOẶC Mất line) thì ngừng tính toán chạy
...
bool XuLyMatLine()
{
    ...
    if (raw_sensor == 13) 
    {
        ...
        // Phanh gấp ngay lập tức sau 50ms
    ...
    else if (abs(raw_sensor) <= 2 || raw_sensor == 14) 
    {
        dang_mat_line = false;
        digitalWrite(pin_bao_mat_line, HIGH); // Tắt còi
...
void XuLyMayTrangThai()
{
    ...
    if ((trang_thai_14 == BAT_DAU_GIAM_TOC || trang_thai_14 == CHO_DUNG) && (millis() - thoi_gian_bat_dau_giam > time_tang_toc_case14_lan_1))
    {
        ...
        t_bat_dau_tam_dung = millis(); 
        dang_tha_hang = true;
        t_bat_dau_tha = millis();
        digitalWrite(pin_dung, LOW); 
    }
    if (trang_thai_14 == TAM_DUNG_DE_THA)
    {
        if (millis() - t_bat_dau_tam_dung >= 500) // Đủ 500ms xả lực thanh kéo
        {
